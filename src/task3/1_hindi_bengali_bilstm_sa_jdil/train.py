...
def train_model(model, optimizer, bengali_dataloader, hindi_dataloader, 
    ...
    for epoch in range(max_epochs):
        ...
        for i, (hindi_train_loader, bengali_train_loader) in enumerate(zip(cycle(hindi_dataloader['train_loader']), bengali_dataloader['train_loader'])):
            ...
            ben_pred, b_annotation_weight_matrix = model(torch.autograd.Variable(bengali_batch).to(device), bengali_lengths.cpu().numpy(), lang='bengali') ## perform forward pass for bengali batch        
            hindi_pred, h_annotation_weight_matrix = model(torch.autograd.Variable(hindi_batch).to(device), hindi_lengths.cpu().numpy(), lang='hindi') ## perform forward pass for hindi batch

            ben_predictions = torch.squeeze(ben_pred) ## get the bengali prediction values
            hindi_predictions = torch.squeeze(hindi_pred) ## get the hindi prediction values
            
            hindi_attention_loss = attention_penalty_loss(h_annotation_weight_matrix, 
                                                    config_dict['self_attention_config']['penalty'], device)
            
            bengali_attention_loss = attention_penalty_loss(b_annotation_weight_matrix, 
                                                    config_dict['self_attention_config']['penalty'], device)

           
            ##  we compute joint loss for both bengali and hindi batches alongside their attention losses.
            loss = criterion(ben_predictions.to(device), torch.autograd.Variable(bengali_targets.float()).to(device)) + \
                   criterion(hindi_predictions.to(device), torch.autograd.Variable(hindi_targets.float()).to(device)) + \
                    hindi_attention_loss + bengali_attention_loss

            loss.backward() ## perform backward pass
            optimizer.step() ## update weights

            hindi_pred_val = hindi_predictions >=0.5 
            hindi_y_true += list(hindi_targets.int().numpy()) ## accumulate targets from batch
            hindi_y_pred += list(hindi_pred_val.data.int().detach().cpu().numpy()) ## accumulate preds from batch 
            total_loss += loss ## accumulate joint train loss

            bengali_pred_val = ben_predictions >=0.5 
            bengali_y_true += list(bengali_targets.int().numpy()) ## accumulate targets from batch
            bengali_y_pred += list(bengali_pred_val.data.int().detach().cpu().numpy()) ## accumulate preds from batch 
