...
def evaluate_bengali_test_set(model, model_name, data, data_loader, device):
    """
    ...
    try:
        ...
        print('best bengali model loaded...')
    except:
        print('no bengali prior model')

    model.eval()
    logging.info('Evaluating accuracy on Bengali test set')
    ...
    for batch, targets, lengths, raw_data in data_loader['test_loader']:
        ...
        pred, annotation_weight_matrix = model(torch.autograd.Variable(batch).to(device), lengths.cpu().numpy(), lang='bengali') ## perform forward pass                    
